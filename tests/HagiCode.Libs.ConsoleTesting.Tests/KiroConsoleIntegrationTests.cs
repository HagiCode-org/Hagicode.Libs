...
using HagiCode.Libs.Kiro.Console;
...
public sealed class KiroConsoleIntegrationTests
{
    private const string RealCliTestsEnvironmentVariable = "HAGICODE_REAL_CLI_TESTS";

    [Fact]
    public async Task DispatchAsync_runs_kiro_default_suite_with_fake_provider()
    {
        var provider = new FakeKiroProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new KiroConsoleRunner(KiroConsoleDefinition.Instance, provider, formatter);

        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync([], KiroConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        var rendered = output.ToString();
        rendered.ShouldContain("[PASS] kiro / Ping");
        rendered.ShouldContain("[PASS] kiro / Simple Prompt");
        rendered.ShouldContain("[PASS] kiro / Complex Prompt");
        rendered.ShouldContain("[PASS] kiro / Session Resume");
    ...
    public async Task DispatchAsync_shows_provider_specific_help_text()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--help"], KiroConsoleDefinition.Instance, runner, output);
        ...
        rendered.ShouldContain("--auth-token <token>");
        rendered.ShouldContain("--arg <value>");
    }

    [Fact]
    public async Task DispatchAsync_normalizes_kiro_aliases_for_provider_commands()
    {
        var provider = new FakeKiroProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "kiro-cli"], KiroConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        output.ToString().ShouldContain("[PASS] kiro / Ping");
    }
    ...
    public async Task DispatchAsync_rejects_foreign_provider_names()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "codex"], KiroConsoleDefinition.Instance, runner, output);
    ...
    [Fact]
    public async Task DispatchAsync_reports_configuration_failures_for_invalid_kiro_options()
    {
        var provider = new FakeKiroProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ...
            KiroConsoleDefinition.Instance,
            runner,
            output);
    ...
    [Fact]
    public async Task DispatchAsync_passes_provider_specific_runtime_options_to_scenarios()
    {
        var provider = new FakeKiroProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new KiroConsoleRunner(KiroConsoleDefinition.Instance, provider, formatter);
        var repositoryPath = Path.Combine(Path.GetTempPath(), $"kiro-console-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            output.ToString().ShouldContain("[PASS] kiro / Repository Summary");
        }
        finally
        {
    ...
    public async Task ProgramMain_can_ping_the_real_kiro_cli_when_opted_in()
    {
    ...
    private sealed class FakeKiroProvider : ICliProvider<KiroOptions>
    {
        ...
        public List<KiroOptions> ReceivedOptions { get; } = [];
        ...
        private string BuildResponse(string prompt, KiroOptions options, string sessionId)
        {
