...
using HagiCode.Libs.OpenCode.Console;
...
public sealed class OpenCodeConsoleIntegrationTests
{
    private const string RealCliTestsEnvironmentVariable = "HAGICODE_REAL_CLI_TESTS";

    [Fact]
    public async Task DispatchAsync_runs_opencode_default_suite_with_fake_provider()
    {
        var provider = new FakeOpenCodeProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new OpenCodeConsoleRunner(OpenCodeConsoleDefinition.Instance, provider, formatter);

        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync([], OpenCodeConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        var rendered = output.ToString();
        rendered.ShouldContain("[PASS] opencode / Ping");
        rendered.ShouldContain("[PASS] opencode / Simple Prompt");
        rendered.ShouldContain("[PASS] opencode / Complex Prompt");
        rendered.ShouldContain("[PASS] opencode / Session Resume");
    ...
    public async Task DispatchAsync_shows_provider_specific_help_text()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--help"], OpenCodeConsoleDefinition.Instance, runner, output);
        ...
        rendered.ShouldContain("--base-url <url>");
        rendered.ShouldContain("--workspace <id>");
        rendered.ShouldContain("--arg <value>");
    }

    [Fact]
    public async Task DispatchAsync_accepts_the_opencode_cli_alias()
    {
        var provider = new FakeOpenCodeProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "opencode-cli"], OpenCodeConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        output.ToString().ShouldContain("[PASS] opencode / Ping");
    }
    ...
    public async Task DispatchAsync_rejects_foreign_provider_names()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "codex"], OpenCodeConsoleDefinition.Instance, runner, output);
    ...
    [Fact]
    public async Task DispatchAsync_reports_configuration_failures_for_invalid_opencode_options()
    {
        var provider = new FakeOpenCodeProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ...
            OpenCodeConsoleDefinition.Instance,
            runner,
            output);
    ...
    public async Task DispatchAsync_passes_default_and_explicit_runtime_options_to_scenarios()
    {
        ...
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new OpenCodeConsoleRunner(OpenCodeConsoleDefinition.Instance, provider, formatter);
        var repositoryPath = Path.Combine(Path.GetTempPath(), $"opencode-console-{Guid.NewGuid():N}");
        Directory.CreateDirectory(repositoryPath);

        try
        {
            var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
                ["--test-provider-full", "--repo", repositoryPath, "--executable", "/tmp/opencode", "--arg", "--profile=smoke"],
                OpenCodeConsoleDefinition.Instance,
                runner,
                output);
            ...
            provider.ReceivedOptions[0].ExecutablePath.ShouldBe("/tmp/opencode");
            ...
            output.ToString().ShouldContain("[PASS] opencode / Repository Summary");
        }
        finally
        {
        ...
        provider = new FakeOpenCodeProvider();
        using var explicitOutput = new StringWriter();
        formatter = new ProviderConsoleOutputFormatter(explicitOutput);
        runner = new OpenCodeConsoleRunner(OpenCodeConsoleDefinition.Instance, provider, formatter);

        var explicitExitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ["--test-provider-full", "--model", "anthropic/claude-sonnet-4", "--base-url", "http://127.0.0.1:4096", "--workspace", "team-a"],
            OpenCodeConsoleDefinition.Instance,
            runner,
            explicitOutput);

        explicitExitCode.ShouldBe(0);
        provider.ReceivedOptions[0].Model.ShouldBe("anthropic/claude-sonnet-4");
        provider.ReceivedOptions[0].BaseUrl.ShouldBe("http://127.0.0.1:4096");
        provider.ReceivedOptions[0].Workspace.ShouldBe("team-a");
    }

    [Fact]
    public async Task DispatchAsync_rejects_removed_fixture_option()
    {
        var provider = new FakeOpenCodeProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ["--test-provider-full", "--fixture"],
            OpenCodeConsoleDefinition.Instance,
            runner,
            output);

        exitCode.ShouldBe(1);
        output.ToString().ShouldContain("Unknown option: --fixture");
    }

    [Fact]
    [Trait("Category", "RealCli")]
    public async Task ProgramMain_can_ping_the_real_opencode_cli_when_opted_in()
    {
    ...
    private sealed class FakeOpenCodeProvider : ICliProvider<OpenCodeOptions>
    {
        ...
        public List<OpenCodeOptions> ReceivedOptions { get; } = [];
        ...
        private string BuildResponse(string prompt, OpenCodeOptions options, string sessionId)
        {
