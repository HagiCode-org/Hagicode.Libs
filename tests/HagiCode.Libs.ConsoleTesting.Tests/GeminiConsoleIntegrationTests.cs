...
using HagiCode.Libs.Gemini.Console;
...
public sealed class GeminiConsoleIntegrationTests
{
    private const string RealCliTestsEnvironmentVariable = "HAGICODE_REAL_CLI_TESTS";

    [Fact]
    public async Task DispatchAsync_runs_gemini_default_suite_with_fake_provider()
    {
        var provider = new FakeGeminiProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new GeminiConsoleRunner(GeminiConsoleDefinition.Instance, provider, formatter);

        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync([], GeminiConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        var rendered = output.ToString();
        rendered.ShouldContain("[PASS] gemini / Ping");
        rendered.ShouldContain("[PASS] gemini / Simple Prompt");
        rendered.ShouldContain("[PASS] gemini / Complex Prompt");
        rendered.ShouldContain("[PASS] gemini / Session Resume");
    ...
    public async Task DispatchAsync_shows_provider_specific_help_text()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--help"], GeminiConsoleDefinition.Instance, runner, output);
    ...
    [Fact]
    public async Task DispatchAsync_accepts_the_gemini_cli_alias()
    {
        var provider = new FakeGeminiProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "gemini-cli"], GeminiConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        output.ToString().ShouldContain("[PASS] gemini / Ping");
    }
    ...
    public async Task DispatchAsync_rejects_foreign_provider_names()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "codex"], GeminiConsoleDefinition.Instance, runner, output);
    ...
    [Fact]
    public async Task DispatchAsync_reports_configuration_failures_for_invalid_gemini_options()
    {
        var provider = new FakeGeminiProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ...
            GeminiConsoleDefinition.Instance,
            runner,
            output);
    ...
    public async Task DispatchAsync_passes_default_and_explicit_runtime_options_to_scenarios()
    {
        ...
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new GeminiConsoleRunner(GeminiConsoleDefinition.Instance, provider, formatter);
        var repositoryPath = Path.Combine(Path.GetTempPath(), $"gemini-console-{Guid.NewGuid():N}");
        Directory.CreateDirectory(repositoryPath);

        try
        {
            var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
                ["--test-provider-full", "--repo", repositoryPath, "--executable", "/tmp/gemini", "--arg", "--profile=smoke"],
                GeminiConsoleDefinition.Instance,
                runner,
                output);
            ...
            provider.ReceivedOptions[0].ExecutablePath.ShouldBe("/tmp/gemini");
            ...
            output.ToString().ShouldContain("[PASS] gemini / Repository Summary");
        }
        finally
        {
        ...
        runner = new GeminiConsoleRunner(GeminiConsoleDefinition.Instance, provider, formatter);

        var explicitExitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ["--test-provider-full", "--model", "gemini-k2.5", "--auth-method", "token", "--auth-token", "secret"],
            GeminiConsoleDefinition.Instance,
            runner,
            explicitOutput);

        explicitExitCode.ShouldBe(0);
        provider.ReceivedOptions[0].Model.ShouldBe("gemini-k2.5");
    ...
    public async Task ProgramMain_can_ping_the_real_gemini_cli_when_opted_in()
    {
    ...
    private sealed class FakeGeminiProvider : ICliProvider<GeminiOptions>
    {
        ...
        public List<GeminiOptions> ReceivedOptions { get; } = [];
        ...
        private string BuildResponse(string prompt, GeminiOptions options, string sessionId)
        {
