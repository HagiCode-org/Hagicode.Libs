...
using HagiCode.Libs.IFlow.Console;
...
public sealed class IFlowConsoleIntegrationTests
{
    private const string RealCliTestsEnvironmentVariable = "HAGICODE_REAL_CLI_TESTS";

    [Fact]
    public async Task DispatchAsync_runs_iflow_default_suite_with_fake_provider()
    {
        var provider = new FakeIFlowProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new IFlowConsoleRunner(IFlowConsoleDefinition.Instance, provider, formatter);

        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync([], IFlowConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        var rendered = output.ToString();
        rendered.ShouldContain("[PASS] iflow / Ping");
        rendered.ShouldContain("[PASS] iflow / Simple Prompt");
        rendered.ShouldContain("[PASS] iflow / Complex Prompt");
        rendered.ShouldContain("[PASS] iflow / Session Resume");
    ...
    public async Task DispatchAsync_shows_provider_specific_help_text()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--help"], IFlowConsoleDefinition.Instance, runner, output);
        ...
        rendered.ShouldContain("--endpoint <ws-url>");
    }

    [Fact]
    public async Task DispatchAsync_normalizes_iflow_aliases_for_provider_commands()
    {
        var provider = new FakeIFlowProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "iflow-cli"], IFlowConsoleDefinition.Instance, runner, output);

        exitCode.ShouldBe(0);
        output.ToString().ShouldContain("[PASS] iflow / Ping");
    }
    ...
    public async Task DispatchAsync_rejects_foreign_provider_names()
    {
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(["--test-provider", "codex"], IFlowConsoleDefinition.Instance, runner, output);
    ...
    [Fact]
    public async Task DispatchAsync_reports_configuration_failures_for_invalid_iflow_options()
    {
        var provider = new FakeIFlowProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        ...
        var exitCode = await ProviderConsoleCommandDispatcher.DispatchAsync(
            ["--test-provider-full", "--endpoint", "http://example.com"],
            IFlowConsoleDefinition.Instance,
            runner,
            output);

        exitCode.ShouldBe(1);
        output.ToString().ShouldContain("Unsupported endpoint");
    }

    [Fact]
    public async Task DispatchAsync_passes_iflow_specific_runtime_options_to_scenarios()
    {
        var provider = new FakeIFlowProvider();
        using var output = new StringWriter();
        var formatter = new ProviderConsoleOutputFormatter(output);
        var runner = new IFlowConsoleRunner(IFlowConsoleDefinition.Instance, provider, formatter);
        var repositoryPath = Path.Combine(Path.GetTempPath(), $"iflow-console-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            provider.ReceivedOptions[0].Model.ShouldBe("iflow/default");
            provider.ReceivedOptions[0].Endpoint.ShouldBe(new Uri("ws://127.0.0.1:7331/acp"));
            provider.ReceivedOptions[0].ExecutablePath.ShouldBe("/custom/iflow");
            ...
            output.ToString().ShouldContain("[PASS] iflow / Repository Summary");
        }
        finally
        {
    ...
    public async Task ProgramMain_can_ping_the_real_iflow_cli_when_opted_in()
    {
    ...
    private sealed class FakeIFlowProvider : ICliProvider<IFlowOptions>
    {
        ...
        public List<IFlowOptions> ReceivedOptions { get; } = [];
        ...
        private string BuildResponse(string prompt, IFlowOptions options, string sessionId)
        {
