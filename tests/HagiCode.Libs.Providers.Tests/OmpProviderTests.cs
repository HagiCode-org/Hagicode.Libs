...
public sealed class OmpProviderTests
{
    private const string RealCliTestsEnvironmentVariable = "HAGICODE_REAL_CLI_TESTS";
    private static readonly string[] OmpExecutableCandidates = ["omp", "omp-cli"];
    ...
    public async Task ExecuteAsync_builds_expected_non_interactive_json_command()
    {
        ...
        processManager.LastContext.ExecutablePath.ShouldBe("/custom/omp");
        processManager.LastContext.WorkingDirectory.ShouldBe("/tmp/omp-project");
        ...
        processManager.LastContext.EnvironmentVariables!.ShouldContainKeyAndValue("OMP_OFFLINE", "1");
    ...
    public async Task ExecuteAsync_rejects_invalid_thinking_level_without_starting_a_process()
    {
        ...
        exception.Message.ShouldContain("Invalid OMP thinking level 'balanced'");
    ...
    [Fact]
    public async Task ExecuteAsync_normalizes_omp_json_events_into_shared_cli_messages()
    {
        var processManager = new StubCliProcessManager
        {
            ExecuteResult = CreateSuccessfulExecutionResult("Trip outline")
    ...
    public async Task ExecuteAsync_marks_matching_requested_session_id_as_resumed()
    {
        var processManager = new StubCliProcessManager
        {
            ExecuteResult = CreateSuccessfulExecutionResult("Trip outline", "omp-session-1")
        };
        var provider = CreateProvider(processManager: processManager);
        ...
        messages[0].Content.GetProperty("session_id").GetString().ShouldBe("omp-session-1");
        messages[0].Content.GetProperty("requested_session_id").GetString().ShouldBe("omp-session-1");
    ...
    public async Task ExecuteAsync_surfaces_non_zero_exit_as_terminal_failure_with_diagnostics()
    {
        ...
        var messages = await CollectMessagesAsync(provider, new OmpOptions { Model = "glm/glm-4.7" }, "Plan a trip.");
    ...
    public async Task ExecuteAsync_reports_invalid_json_output_as_terminal_failure()
    {
        ...
        var messages = await CollectMessagesAsync(provider, new OmpOptions { NoSession = true }, "Plan a trip.");
    ...
    public async Task PingAsync_reports_version_when_process_succeeds()
    {
        ...
        result.ProviderName.ShouldBe("omp");
    ...
    public async Task ExecuteAsync_real_cli_trip_prompt_returns_assistant_output_or_actionable_failure_when_opted_in()
    {
        ...
        var executablePath = resolver.ResolveFirstAvailablePath(OmpExecutableCandidates);
        if (executablePath is null)
        {
            throw new InvalidOperationException("OMP CLI was not found on PATH even though the real CLI lane was enabled.");
        }

        using var workspace = new TemporaryOmpWorkspace();
        await using var provider = new OmpProvider(resolver, new CliProcessManager());
        ...
        RealCliInvocationTestHarness.AssertActionableFailure("omp", failureMessage);
    }

    private static OmpProvider CreateProvider(
    ...
    private static async Task<List<CliMessage>> CollectMessagesAsync(
        ICliProvider<OmpOptions> provider,
        OmpOptions options,
        string prompt,
    ...
    private static ProcessResult CreateSuccessfulExecutionResult(string assistantText, string sessionId = "session-1")
    {
        ...
        var lines = new[]
        {
            JsonSerializer.Serialize(new { type = "session", version = 3, id = sessionId, timestamp = "2026-06-06T13:03:16.799Z", cwd = "/tmp/omp-project" }),
    ...
    private static ProcessResult CreateFailedExecutionResult(string errorMessage, string standardError)
    {
        ...
        var lines = new[]
        {
            ...
            JsonSerializer.Serialize(new { type = "session", version = 3, id = "session-1", timestamp = "2026-06-06T13:03:32.219Z", cwd = "/tmp/omp-project" }),
            JsonSerializer.Serialize(new { type = "message_end", message = assistantMessage }),
            JsonSerializer.Serialize(new { type = "turn_end", message = assistantMessage, toolResults = Array.Empty<object>() }),
    ...
    private static ProcessResult CreateStreamingExecutionResult(string sessionId = "session-stream")
    {
        ...
        var lines = new[]
        {
            JsonSerializer.Serialize(new { type = "session", version = 3, id = sessionId, timestamp = "2026-06-07T09:22:21.958Z", cwd = "/tmp/omp-project" }),
            JsonSerializer.Serialize(new
            {
                type = "message_update",
    ...
    private static ProcessResult CreateWhitespaceStreamingExecutionResult(string sessionId = "session-whitespace")
    {
        ...
        {
            JsonSerializer.Serialize(new { type = "session", version = 3, id = sessionId, timestamp = "2026-06-07T12:00:00.000Z", cwd = "/tmp/omp-project" })
        };
    ...
    private static ProcessResult CreateToolCallExecutionResult(string sessionId = "session-tool")
    {
        var toolUseAssistant = new
        {
            ...
            content = new object[]
            {
                ...
                new
                {
                    ...
                    arguments = new
                    {
                        path = "/tmp/omp-project"
                    },
                    partialArgs = "{\"path\":\"/tmp/omp-project\"}",
                    streamIndex = 0
                }
            },
            provider = "omniroute",
        ...
        var lines = new[]
        {
            JsonSerializer.Serialize(new { type = "session", version = 3, id = sessionId, timestamp = "2026-06-07T09:39:20.000Z", cwd = "/tmp/omp-project" }),
            JsonSerializer.Serialize(new
            {
                type = "message_update",
                assistantMessageEvent = new
    ...
    private static ProcessResult CreateCrossTurnReplayExecutionResult(string sessionId = "session-replay")
    {
        ...
        var lines = new[]
        {
            JsonSerializer.Serialize(new { type = "session", version = 3, id = sessionId, timestamp = "2026-06-07T13:00:00.000Z", cwd = "/tmp/omp-project" }),
            JsonSerializer.Serialize(new
            {
                type = "message_update",
                assistantMessageEvent = new
    ...
    private sealed class StubCliProcessManager : CliProcessManager
    {
        ...
        {
            var tempDirectory = Path.Combine(Path.GetTempPath(), $"hagicode-libs-omp-stub-{Guid.NewGuid():N}");
...
";
    ...
    private sealed class TemporaryOmpWorkspace : IDisposable
    {
        private bool _disposed;

        public TemporaryOmpWorkspace()
        {
            RootDirectory = Path.Combine(Path.GetTempPath(), $"hagicode-libs-omp-{Guid.NewGuid():N}");
            WorkingDirectory = Path.Combine(RootDirectory, "workspace");

            Directory.CreateDirectory(WorkingDirectory);
            File.WriteAllText(Path.Combine(WorkingDirectory, "README.md"), "# omp real cli workspace");
        }
