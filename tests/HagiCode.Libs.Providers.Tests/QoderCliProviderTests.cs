...
public sealed class QoderCliProviderTests
{
    ...
    public async Task ExecuteAsync_reuses_pooled_session_without_reconnecting_when_pooling_is_enabled()
    {
        ...
        await foreach (var _ in provider.ExecuteAsync(new QoderCliOptions { SessionId = "session-key" }, "first"))
        {
        }

        await foreach (var _ in provider.ExecuteAsync(new QoderCliOptions { SessionId = "session-key" }, "second"))
        {
        }
