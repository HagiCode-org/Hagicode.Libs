...
public sealed class KimiProviderTests
{
    ...
    private static readonly string[] KimiExecutableCandidates = ["kimi", "kimi-cli"];
    ...
    public void BuildCommandArguments_trims_tokens_and_omits_duplicate_acp_entries()
    {
        ...
        var arguments = provider.BuildCommandArguments(new KimiOptions
        {
    ...
    public async Task ExecuteAsync_uses_custom_executable_authenticates_and_streams_normalized_messages()
    {
        ...
        provider.LastStartContext!.ExecutablePath.ShouldBe("/custom/kimi");
        ...
        provider.LastStartContext.EnvironmentVariables!["KIMI_PROFILE"].ShouldBe("ci");
        ...
        provider.SessionClient.LastModel.ShouldBe("kimi-k2.5");
    ...
    public async Task ExecuteAsync_authentication_failure_surfaces_bootstrap_context()
    {
        ...
        {
            await foreach (var _ in provider.ExecuteAsync(new KimiOptions(), "hello"))
            {
            }
        });
    ...
    public async Task PingAsync_reports_initialize_details_when_bootstrap_succeeds()
    {
        ...
        result.Version.ShouldContain("kimi");
        provider.SessionClient!.InitializeCalls.ShouldBe(1);
    }
    ...
    public async Task PingAsync_can_validate_installed_kimi_cli_when_opted_in()
    {
        ...
        var executablePath = resolver.ResolveFirstAvailablePath(KimiExecutableCandidates);
        if (executablePath is null)
        {
            throw new InvalidOperationException("Kimi CLI was not found on PATH even though the real CLI validation path was enabled.");
        }

        var provider = new KimiProvider(resolver, new CliProcessManager(), null);

        provider.IsAvailable.ShouldBeTrue();

        var result = await provider.PingAsync();

        result.ProviderName.ShouldBe("kimi");
        result.Success.ShouldBeTrue();
        result.Version.ShouldNotBeNullOrWhiteSpace();
        result.ErrorMessage.ShouldBeNullOrWhiteSpace();
    }

    private static TestKimiProvider CreateProvider(
        CliExecutableResolver? executableResolver = null,
        FakeAcpSessionClient? sessionClient = null)
    {
        return new TestKimiProvider(
    ...
    private sealed class TestKimiProvider(
        CliExecutableResolver executableResolver,
        CliProcessManager processManager,
        IRuntimeEnvironmentResolver runtimeEnvironmentResolver,
        FakeAcpSessionClient sessionClient)
        : KimiProvider(executableResolver, processManager, runtimeEnvironmentResolver)
    {
