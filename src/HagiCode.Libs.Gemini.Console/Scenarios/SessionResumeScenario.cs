...
public static class SessionResumeScenario
{
    public static ProviderConsoleScenario<ICliProvider<GeminiOptions>> Create(GeminiConsoleExecutionOptions executionOptions)
    {
        ...
        return new ProviderConsoleScenario<ICliProvider<GeminiOptions>>(
            "Session Resume",
            "Verify that a follow-up request can resume the same Gemini session.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        var initialResult = await GeminiScenarioMessageReader.ReadExecutionResultAsync(
            provider,
        ...
        if (string.IsNullOrWhiteSpace(initialResult.SessionId))
        {
            return new ProviderConsoleScenarioResult(
                ...
                false,
                0,
                ErrorMessage: "Gemini did not expose a session identifier, so the session could not be resumed.");
        }
        ...
        var followUpResult = await GeminiScenarioMessageReader.ReadExecutionResultAsync(
            provider,
            resumedOptions,
