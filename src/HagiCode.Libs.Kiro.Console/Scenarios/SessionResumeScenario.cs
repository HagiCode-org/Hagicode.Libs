...
public static class SessionResumeScenario
{
    public static ProviderConsoleScenario<ICliProvider<KiroOptions>> Create(KiroConsoleExecutionOptions executionOptions)
    {
        ...
        return new ProviderConsoleScenario<ICliProvider<KiroOptions>>(
            "Session Resume",
            "Verify that a follow-up request can resume the same Kiro session.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        var initialResult = await KiroScenarioMessageReader.ReadExecutionResultAsync(
            provider,
        ...
        if (string.IsNullOrWhiteSpace(initialResult.SessionId))
        {
            return new ProviderConsoleScenarioResult(
                ...
                false,
                0,
                ErrorMessage: "Kiro did not expose a session identifier, so the session could not be resumed.");
        }
        ...
        var followUpResult = await KiroScenarioMessageReader.ReadExecutionResultAsync(
            provider,
            resumedOptions,
