...
public static class SessionResumeScenario
{
    public static ProviderConsoleScenario<ICliProvider<CodexOptions>> Create(CodexConsoleExecutionOptions executionOptions)
    {
        ...
        return new ProviderConsoleScenario<ICliProvider<CodexOptions>>(
            "Session Resume",
            "Verify that a follow-up request can resume the same Codex thread.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        var initialResult = await CodexScenarioMessageReader.ReadExecutionResultAsync(
            provider,
        ...
        var initialCombined = string.Join(" ", initialResult.Messages);
        ...
        if (string.IsNullOrWhiteSpace(initialResult.ThreadId))
        {
            return new ProviderConsoleScenarioResult(
                provider.Name,
                "Session Resume",
                false,
                0,
                ErrorMessage: "Codex did not emit a thread.started event, so the session could not be resumed.");
        }

        var resumedOptions = executionOptions.CreateBaseOptions() with
        {
            ThreadId = initialResult.ThreadId,
        };

        var followUpMessages = await CodexScenarioMessageReader.ReadAssistantMessagesAsync(
            provider,
            resumedOptions,
        ...
        var followUpCombined = string.Join(" ", followUpMessages);
